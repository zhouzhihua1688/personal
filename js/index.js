...
// you导航的显示与隐藏
 	$('.rnavbtn').click(function(){
        if( tag==0 ){
            ...
            $(this).html( '《' )
            $(this).css('left',-6)
            $('#level').animate({'right':100},500)
            $('#level').animate({'opacity':0.36},500)
    		$('.btn li').animate({'opacity':0.36},500)
        }else{
            $(this).html( '》' )
            $(this).css('left',0)
        	tag=0
            $('#level').animate({'right':286,'opacity':1},500)
    		$('.btn li').animate({'opacity':1},500)
        }
    });
 	function rnavg(){
        if( tag==0 ){
 	...
 	$(document).mousewheel(function(e,delta){
 		if(!$('.wrip').is(':animated')&falgqq ==0){
 			...
 			if(!num==0){
				...
				rnavg();
				// $('.aembed').css({'height':'80','width':'140'})
			}else{
...
// 第二屏开始，第二屏背景
    ...
    $('.boxcent .r').mouseover(function(){
    	falgqq=1;
    	$('.boxcenbg').stop().animate({'opacity':0},500);
    	// $('.wrip .page2 .pave li.num3 p').css('display',block)
    }).mouseout(function(){
    	falgqq=0;
    	$('.boxcenbg').stop().animate({'opacity':0.5},500);
    ...
    // 第二屏幕滚动
    	var json0 = {'width':54,'height':29,'left':-84,}
		var json1 = {'width':77,'height':64,'left':-2,}
		var json2 = {'width':122,'height':96,'left':85,}
		var json3 = {'width':188,'height':200,'left':232,}
		var json4 = {'width':122,'height':96,'left':439,}
		var json5 = {'width':77,'height':64,'left':577,}
		var json6 = {'width':54,'height':29,'left':693,}
		...
		$(".boxp2").mouseover(function(){
			...
			console.log(c_hh)
			$('.btn2 span').stop().animate({'opacity':0.8},500)
		...
		$(".boxp2").mouseout(function(){
			...
			$('.btn2 span').stop().animate({'opacity':0},500)
    		$('.boxcenbg').stop().animate({'opacity':0.5},500);
...
// 第三屏--------------------
	...
	for(var i = 9 ; i > 0 ; i--){
		var temp = {"width":100,"height":100,"left":1280 - 100 * i,'bottom':20};
		muArray.push(temp);
	}
	muArray.push({"width":241,"height":428,"left":78,'bottom':5});
	for(var i = 0 ; i < 9 ; i++){
		var temp = {"width":100,"height":100,"left":380 + 100 * i,'bottom':30};
		muArray.push(temp);
	}
...
// 转盘
	...
	$('.zhuanp .pan,.zhuanp .box').mouseover(function(){
		...
		$('.zhuanp span').stop().animate({'opacity':0.4},500)
	}).mouseout(function(){
		timer06=setInterval(autoplay,2000)
		$('.zhuanp span').stop().animate({'opacity':0},500)
...
// 第五屏
    ...
    setInterval(function autopaly(){
	    // if(!$('.banner li').is(":animated")){
	        ...
	        // $('.banner li').eq(numbanner).addClass('wenziact').siblings().removeClass('wenziact');
